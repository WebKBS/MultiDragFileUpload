...
function viewImageHandler(fileList, targetElement) {
  ...
  reader.addEventListener('load', async (event) => {
    const elements = await createElementImage(event);
    const image = elements.querySelector('img');

    if (targetElement.target.id === 'upload') {
      if (!invaildImage1920(image)) return false;
      return;
    }

    if (targetElement.target.id === 'upload1') {
      if (!invaildImage1024(image)) return false;
      return;
    }

    if (targetElement.target.id === 'upload2') {
      if (!invaildImage1024(image)) return false;
      return;
    }
...
/**
 * 이미지 1920 x 760 사이즈 유효성 검사
 * @param data - 이미지 파일
 * */
function invaildImage1920(data) {
  if (data.naturalWidth !== 1920 && data.naturalHeight !== 760) {
    Swal.fire({
      text: '이미지 1920 크기가 상이합니다. 적합한 이미지를 등록해주세요.',
      icon: 'error',
    });
    return false;
  }
...
/**
 * 이미지 1024 x 640 사이즈 유효성 검사
 * @param data - 이미지 파일
 * */
function invaildImage1024(data) {
  if (data.naturalWidth !== 1024 && data.naturalHeight !== 640) {
    Swal.fire({
      text: '이미지 1024 크기가 상이합니다. 적합한 이미지를 등록해주세요.',
      icon: 'error',
    });
    return false;
  }
